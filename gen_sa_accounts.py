...
def auth_step1(
    ...
    flow = InstalledAppFlow.from_client_secrets_file(credentials, SCOPES, redirect_uri='urn:ietf:wg:oauth:2.0:oob')
    global current_flow
    current_flow = flow
...
def auth_step2(
    ...
    global current_flow
    current_flow.fetch_token(code=code)
    creds = current_flow.credentials
    with open(token, 'wb') as t:
        pickle.dump(creds, t)
